...
class LinearMRF(object):
    ...
    def build_training_obj(self, img_features, unary_beliefs, pair_beliefs,
        ...
        for i in range(len(unary_beliefs)):
            unary_part = tf.reduce_sum(tf.multiply(unary_beliefs[i], unary_potentials[i]))
            pair_part = tf.reduce_sum(tf.multiply(pair_beliefs[i], pairwise_potentials))
            F_unary = tf.reduce_sum(tf.multiply(self.unary_weight, img_features))
            F_pair = tf.reduce_sum(tf.multiply(self.pairwise_weight, self.get_pairwise_beliefs(img_features)))
            obj += tf.cast(unary_part, tf.float32) + tf.cast(pair_part, tf.float32) - F_unary - F_pair
            # F_unary = tf.reduce_sum(tf.multiply(unary_potentials[i], img_features))
            # F_pair = tf.reduce_sum(tf.multiply(pairwise_potentials, self.get_pairwise_beliefs(img_features)))
            # obj += unary_part + pair_part - F_unary - F_pair
            F_unary = tf.reduce_sum(self.calculate_unary_potentials(img_features))
            F_pair = tf.reduce_sum(self.calculate_pairwise_potentials(self.get_pairwise_beliefs(img_features)))
            obj += tf.cast(unary_part, tf.float32) + tf.cast(pair_part, tf.float32) - F_unary - F_pair
    ...
    def inference_itr(self, unary_beliefs, unary_pots, pairwise_pots):
        ...
        inference_result = np.zeros((unary_beliefs.shape))
        random_order = np.random.permutation(range(len(unary_beliefs)))
        for i in random_order:
            ass0_score = self.calculate_local_score(i, 0, unary_beliefs, unary_pots, pairwise_pots)
            ass1_score = self.calculate_local_score(i, 1, unary_beliefs, unary_pots, pairwise_pots)
            if (ass0_score > ass1_score):
                inference_result[i, 0] = 1
            else:
                inference_result[i, 1] = 1
    ...
    def calculate_local_score(self, node, assignment, unary_beliefs,
        ...
        '''
        score = 0.0
        unary_part = unary_potentials[node, assignment]
        score = score + unary_part
        for i in self.neighbors[node]:
            node_assignment = assignment
            neighbor_assignment = np.argmax(unary_beliefs[i])
            if (node < i):  # make sure m<n
                pairwise_pot_col_idx = 2 * node_assignment + neighbor_assignment  # y_m is node_assignment
            else:
                pairwise_pot_col_idx = 2 * neighbor_assignment + node_assignment  # y_m is neighbor_assignment
            pair_score = pairwise_potentials[self.pair_inds[(node, i)], pairwise_pot_col_idx]
            score += pair_score
        return score
    ...
    def check_convergence(self, new_unary_beliefs, old_unary_beliefs,
        ...
        '''
        percentage = np.sum(new_unary_beliefs != old_unary_beliefs) / len(new_unary_beliefs)
        return (percentage < convergence_margin)
    ...
    def get_pairwise_beliefs(self, unary_beliefs):
        ...
        for m, n in self.pairs:
            ...
            if (unary_beliefs[m, 0] == 1 and unary_beliefs[n, 0] == 1):
                pairwise_beliefs[self.pair_inds[(m, n)], 0] = 1
            elif (unary_beliefs[m, 0] == 1 and unary_beliefs[n, 1] == 1):
                pairwise_beliefs[self.pair_inds[(m, n)], 1] = 1
            elif (unary_beliefs[m, 1] == 1 and unary_beliefs[n, 0] == 1):
                pairwise_beliefs[self.pair_inds[(m, n)], 2] = 1
            elif (unary_beliefs[m, 1] == 1 and unary_beliefs[n, 1] == 1):
                pairwise_beliefs[self.pair_inds[(m, n)], 3] = 1
        print(pairwise_beliefs)
